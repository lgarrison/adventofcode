...
";*/
    ...
    for i in 0..nums.len() {
        for j in 0..nums.len() {
            ...
            let mut sum = nums.get_mut(i).unwrap().clone();
            let mut sumd = depths.get_mut(i).unwrap().clone();
            
            let mut num2 = nums.get_mut(j).unwrap().clone();
            let mut d2 = depths.get_mut(j).unwrap().clone();
